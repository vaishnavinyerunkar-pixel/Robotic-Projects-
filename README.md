# 👋 Hi, I’m **Vaishnavi N Yerunkar**  
### 🤖 Robotics | Computer Vision | IoT Developer | Embedded System  

🚀 Building intelligent robotic systems using OpenCV, ESP32, 3D Printing & Embedded Systems.

---

## 🌟 About Me

I design and build real-world robotics and automation systems that combine:

- 🤖 Robotics & Motion Control  
- 🧠 Computer Vision & AI  
- 🌐 IoT & Smart Automation  
- 🔧 Embedded Systems Integration  

I love transforming ideas into intelligent, interactive machines.

---

## 🛠 Tech Stack

### 🔧 Embedded Systems
![ESP32](https://img.shields.io/badge/ESP32-Embedded-red?style=for-the-badge)
![NodeMCU](https://img.shields.io/badge/NodeMCU-ESP8266-blue?style=for-the-badge)
![Arduino](https://img.shields.io/badge/Arduino-Microcontroller-blue?style=for-the-badge)

### 🧠 AI & Computer Vision
![Python](https://img.shields.io/badge/Python-Programming-yellow?style=for-the-badge)
![OpenCV](https://img.shields.io/badge/OpenCV-ComputerVision-green?style=for-the-badge)

### 🌐 IoT & Networking
![IoT](https://img.shields.io/badge/IoT-SmartSystems-orange?style=for-the-badge)
![WiFi](https://img.shields.io/badge/WiFi-Communication-lightgrey?style=for-the-badge)

### 🛠 Tools
![Git](https://img.shields.io/badge/Git-VersionControl-black?style=for-the-badge)
![GitHub](https://img.shields.io/badge/GitHub-Portfolio-purple?style=for-the-badge)

---

## 🚀 Featured Projects

| Project | Description |
|----------|------------|
| ✋ **Robotic Hand (OpenCV Controlled)** | 3D printed robotic hand controlled using real-time computer vision & mobile interface |
| 🚗 **OpenCV & WiFi RC Car** | Object-tracking robotic car with wireless remote functionality |
| 🤖 **Otto Robot – 3D Printed Dance Bot** | Custom-built dancing robot with motion programming |
| 🖥 **Gesture-Controlled PPT System** | Hand gesture-based slide navigation using OpenCV |
| 🌐 **IoT Automation Projects** | ESP32-based smart monitoring and automation systems |

---

## 🔥 What This Portfolio Demonstrates

✔ Real-time Computer Vision → Hardware Integration  
✔ Embedded system architecture  
✔ IoT-based remote control systems  
✔ 3D design & prototyping  
✔ End-to-end robotics development  

---

## 📈 Currently Exploring

- Advanced Computer Vision  
- AI + Robotics Integration  
- ROS (Robot Operating System)  
- Autonomous Navigation  

---

## 📫 Connect With Me

📧 Email: vaishnavinyerunkar@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/vaishnavi-n-yerunkar-1227a528b?utm_source=share_via&utm_content=profile&utm_medium=member_android  
💻 GitHub: https://github.com/vaishnavinyerunkar-pixel  

---

## 🔥 Let’s Build Intelligent Systems Together!

I’m open to:
- Internships  
- Robotics collaborations  
- Research opportunities  
- AI & Embedded Systems projects  

Thank you for visiting my Robotics Portfolio 🚀

🚀 Let’s build the future of Robotics together!
